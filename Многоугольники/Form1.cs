            ...
            if (L.Count > 2) //Если число вершин больше 2-х, запускаем выбор алгоритма построения выпуклой оболочки и сами алгоритмы
            {
                switch (algorithm_choice)
                {
                        ...
                        for (int n = 0; n < L.Count -1; n++)
                        {
                            for (int l = n + 1; l < L.Count; l++)
                            {
                                ...
                                else //нормальный случай, в котором точки не находятся друг над другом по х
                                {
                                    ...
                                    //if ((higher_or_right == true || lower_or_left == true) & !(higher_or_right == true && lower_or_left == true))
                    ...
                    case 1:
                        ...
                        int Biggest_Y;
                        int Lowest_X;
                        // переменные используются только в начале процесса
                        ...
                        float new_Cosine;
                        Biggest_Y = L[0].Y;
                        for(int n=1; n <= L.Count - 1; n ++) //прогоняем весь лист точек, ищем max y;  
                        {
                        if(L[n].Y > Biggest_Y)
                            {
                        ...
                        Lowest_X = L[Index].X;
                        for(int n = 1; n <= L.Count -1; n++) //прогоняем все точки с максимальным y (условно - на одной прямой), ищем самую левую (x - наим)
                        {
                        if(L[n].X < Lowest_X)
                            {
                                Lowest_X = L[n].X;
                        ...
                        Cosine = 0; //начальный косинус, т.к ищем мы косинус изначально наибольший, то cos - минимально возможный
                        for (int i = 0;i <= L.Count -1; i++) //перебор первого угла , сравнение синуса
                        {
                            ...
                            new_Cosine = (float)(x * 2.0 + y * 0)  /(float)(Math.Sqrt(Math.Pow(x, 2.0) + Math.Pow(y, 2.0)) * 2.0);
                        ...
                        while (true) //чтобы не изпользовать рекурсивный метод и не биться головой об стену, прервём построение, когда дойдём до начальной точки
                        {
                            Cosine = 1; //ищём наименьший косинус, поэтому начальный - наибольший из возможных
                            for (int i = 0; i <= L.Count - 1; i++)
                            {
                                if(i == Current_point || i == New_point) //вектор сам на себя не строится
                                {
                                    continue;
                                }
                                int x, y, x1, y1; //Переменные для вычисления векторов
                                //Вектор, который уже известен
                                x = L[Current_point].X - L[New_point].X;
                                y = L[Current_point].Y - L[New_point].Y;
                                //Потенциальный вектор
                                x1 = L[New_point].X - L[i].X;
                                y1 = L[New_point].Y - L[i].Y;
                                new_Cosine = (float)(x * x1 + y * y1) / ((float)(Math.Sqrt(Math.Pow(x, 2.0) + Math.Pow(y, 2.0)) * (float)(Math.Sqrt(Math.Pow(x1, 2.0) + Math.Pow(y1,2.0)))));
                                if(new_Cosine < Cosine)
                                {
                                    Cosine = new_Cosine;
                            ...
                            //Нашли лучшую линию из всех потенциальных, строим
                            ...
                            if (Potential_Point == Index) //новая точка = Index, значит, выходим из цикла
                            {
                                break;
                            }
                            //перестраиваем точки под новое ребро
